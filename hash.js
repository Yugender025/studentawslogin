...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length > 0) {
      ...
      const storedhashPwd = user.password;
      bcrypt.compare(password, storedhashPwd, (err, result) => {
        if (err) {
          console.log("An error occured comaparing:", err);
        } else {
          if (result) {
            res.sendFile(path.join(__dirname, "public", "coursevideos.html"));
          } else {
  ...
  try {
    ...
    if (checkResult.rows.length > 0) {
      ...
      bcrypt.hash(password, saltRounds, async (err, hash) => {
        if (err) {
          console.log("an error occured hashing :", err);
        } else {
          const result = await db.query(
